...
var tabsButton = document.querySelectorAll(".tabs-button");
...
for (let slider of sliders) {
  ...
  for (let radioButton of radioButtons) {
    const currentI = i;
    radioButton.addEventListener('click', () => {
      const currentBtn = slider.querySelector('.radio-control-current');
      const currentSlide = slider.querySelector('.promo-slide-current');
      if (currentBtn) {
        currentBtn.classList.remove('radio-control-current');
      }
      if (currentSlide) {
        currentSlide.classList.remove('promo-slide-current');
      }
...
function setActiveButton(slider, index) {
  const btns = slider.querySelectorAll('.radio-control');
  let i = 0;
  for (let btn of btns) {
    btn.classList.remove('radio-control-current');
    if (i === index) {
      btn.classList.add('radio-control-current');
    }
    i += 1;
  }
}
